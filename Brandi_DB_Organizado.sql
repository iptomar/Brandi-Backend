DROP DATABASE IF EXISTS Brandi_BD;
...
CREATE TABLE Eventos (
    ...
    ID_Pedido INT UNSIGNED NOT NULL,
    CONSTRAINT FK_ID_Pedido_Eventos FOREIGN KEY (ID_Pedido) REFERENCES Pedidos_Clientes(ID_Pedido) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE Designacao_Objecto (
    ...
    /* Connection between Team C DB and Team A DB */
    CONSTRAINT FK_ID_Pedido_Objecto FOREIGN KEY (ID_Pedido) REFERENCES Pedidos_Clientes(ID_Pedido) ON UPDATE CASCADE ON DELETE CASCADE
    /* ########################################## */
);
